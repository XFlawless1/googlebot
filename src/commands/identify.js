...
module.exports = {
  main: async message => {
    ...
    return snekfetch.get('https://www.captionbot.ai/api/init')
    .then((res) => {
      const conversationId = res.body
        .replace('<string xmlns="http://schemas.microsoft.com/2003/10/Serialization/">', '')
        .replace('</string>', '');
      return snekfetch.post('https://www.captionbot.ai/api/message')
        .set({ Accept: 'application/json', 'Content-Type': 'application/json' })
        .send({ conversationId, waterMark: '', userMessage: message.content.replace(/<|>/g, '') })
        .then(() => conversationId);
    })
    .then((cid) => snekfetch.get(`https://www.captionbot.ai/api/message?waterMark=&conversationId=${cid}`))
